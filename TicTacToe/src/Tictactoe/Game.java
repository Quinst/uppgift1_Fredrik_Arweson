...
public class Game {
	...
	private Player[] players =
		{
				...
				//Väljer man att möta en dator laddas det in här vilken symbol den får
		};
	...
	public Game(String player, int grid) {
		...
		clearBoard();
		//Här har vi bestämt symboler för de olika spelarna
	}
	...
	public void move(int xPosition, int yPosition) {
		...
		if (!isGameOver()) {
			currPlayer.playMove(this);
			//Här styr vi själva spelet med symbolerna
		}
	}
	...
	private boolean isValidMove(int xPosition, int yPosition) {
		...
		if  (!board[xPosition][yPosition].equals(FILLER))
			return false;
		return true;
		//Kollar att det är ett godkänt drag
	}
	...
	private void printBoard() {
		...
		System.out.println();
		//Nu skrev vi ut på spelplanen vad det var för drag
	}
	System.out.println(currPlayer.getName() + " Tur nu");
	}
	
	private boolean isGameOver() {
		return getWinner() != null || isNoMovesLeft();
		//Kontrollerar om spelet är klart eller om det skall göras fler drag
	}
	...
	private String diagonalCrossed() {
		...
		if (!check.equals(FILLER)) {
			return check;
			//HEla denna sektion håller koll på om dragen är okej
		}
		return FILLER;
	}
	...
	public Player getWinner( ) {
		...
		return null;
		//Här har vi fått reda på om vi fått en vinnare
	}
